...
def val_split(src_folder, tgt_folder, val_scale=0.1):
   ...
   tgt_gt_dir = os.path.join(tgt_folder, 'GT')
   
   if not os.path.exists(tgt_gt_dir):
       os.mkdir(tgt_gt_dir)
   ...
   selected = indices[:int(len(indices)*val_scale)]
   
   for s in selected:
       move(os.path.join(src_input_dir, files[s]), os.path.join(tgt_input_dir, files[s]))
       move(os.path.join(src_gt_dir, files[s]), os.path.join(tgt_gt_dir, files[s]))
   
dataset = "suid"
src_folder = f"/home/ddimauro/Neptune/Enhancement/Semi-UIR/data/{dataset}/labeled"
target_folder = f"/home/ddimauro/Neptune/Enhancement/Semi-UIR/data/{dataset}/val"
    
if not os.path.exists(target_folder):
    os.mkdir(target_folder)

val_split(src_folder, target_folder)
